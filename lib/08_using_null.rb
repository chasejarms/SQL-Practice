...
def teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      name, COALESCE(mobile, '07986 444 2266') AS phone_number
    FROM
      teachers
  SQL
end
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      COUNT(name) AS teachers_count, COUNT(mobile) AS mobile_phone_count
    FROM
      teachers
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, COUNT(teachers.name) AS staff_count
    FROM
      depts
    LEFT OUTER JOIN
